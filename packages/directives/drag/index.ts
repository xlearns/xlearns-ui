...
const vDrag: ObjectDirective = {
  mounted(el: HTMLElement) {
    const oDiv = el // 当前元素
    const minTop = oDiv.getAttribute('drag-min-top') as unknown as number
    const ifMoveSizeArea = 20
    oDiv.onmousedown = (e) => {
      let target: HTMLElement = oDiv
      while (
      ...
      const initX = parseInt(String(target.getAttribute('init_x')))
      const initY = parseInt(String(target.getAttribute('init_y')))
