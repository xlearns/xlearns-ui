	...
	test('AbsoluteLayoutItem option', () => {
		const wrapper = mount({
			setup() {
				return () => (
					<AbsoluteLayout>
						<AbsoluteLayoutItem option={{width:'150px',x:'100px',y:"-100px"}} ref='item'></AbsoluteLayoutItem>
					</AbsoluteLayout>
				)
			}
		})
		...
		expect(el.style.width=='150px' && el.style.height=='50px' && el.style.left == '100px'&& el.style.top == '-100px').toBe(true)
	})
})
